...
module.exports.createTodo = (event, cb) => {
  helper.createTodo(JSON.parse(event.body)).then(result => {
...
module.exports.updateTodo = (event, cb) => {
  helper.updateTodo(JSON.parse(event.body)).then(result => {
    cb(null, response.create(200, {}));
  }).catch(err => {
    cb(null, response.create(500, {
      err: err
...
module.exports.updateTodoStatus = (event, cb) => {
  helper.updateTodoStatus(JSON.parse(event.body)).then(result => {
    cb(null, response.create(200, {}));
  }).catch(err => {
    cb(null, response.create(500, {
      err: err
...
module.exports.deleteTodo = (event, cb) => {
  helper.deleteTodo(event.pathParameters).then(result => {
    cb(null, response.create(200, {}));
  }).catch(err => {
    cb(null, response.create(500, {
      err: err
